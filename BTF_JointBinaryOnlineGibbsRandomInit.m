function [U lambda pr co_lambda co_pr eva time_trace] = BTF_JointBinaryOnlineGibbsRandomInit(N,co_xi,co_id,xi,id,R,batchsize,co_batchsize,numiters,isbatch,co_isbatch,isnetwork,stentity,fraction)
    K=length(id);
%     for k=1:K 
%         N(k) = max(id{k}); 
%     end
...
%             co_xiselectlatent=truncated_Poisson_rnd_1(co_xi',co_zetai);
        ...
        %%
        %turn off evaluation for fast running
